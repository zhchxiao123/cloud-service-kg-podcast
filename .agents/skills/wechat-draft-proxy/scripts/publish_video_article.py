...
def extract_first_frame(video_path: str) -> str:
    """用 ffmpeg 抽视频首帧作封面,返回临时文件路径"""
    import subprocess
    import tempfile
    out = tempfile.NamedTemporaryFile(suffix=".jpg", delete=False)
    out.close()
    cmd = ["ffmpeg", "-y", "-i", video_path, "-ss", "00:00:01", "-vframes", "1", "-q:v", "2", out.name]
    try:
...
def build_video_html(video_url: str, title: str, description: str,
    ...
    """
    生成嵌视频的 HTML 正文 —— 订阅号可用版本

    微信编辑器对 <video> 标签支持极差(经常被静默删除或替换),订阅号没 mpvideo 群发权限,
    所以采用 "封面 + 按钮链接" 方案:
      - 封面图(微信对 <img> 100% 友好)
      - 醒目的"▶ 点击播放"按钮,链接到 mpvideo.qpic.cn 的 down_url
      - 微信内打开链接会被微信接管,用内置播放器播放
    """
...
'''
    ...
    # 微信里的 mpvideo.qpic.cn 链接,会触发微信内置视频播放器
    return f'''
<section style="margin: 24px 0; padding: 0;">
  <p style="font-size: 20px; font-weight: bold; color: #222; margin: 0 0 16px 0; text-align: center;">{safe_title}</p>
  {f'<p style="text-align: center; margin: 0 0 16px 0;"><img src="{poster}" alt="{safe_title}" style="max-width: 100%; height: auto; border-radius: 8px; display: block; margin: 0 auto;" /></p>' if poster else f'''
  <p style="text-align: center; margin: 0 0 16px 0;">
    <a href="{video_url}" target="_blank" style="display: inline-block; position: relative; max-width: 100%; text-decoration: none;">
      <span style="display: block; width: 240px; height: 240px; max-width: 100%; margin: 0 auto; background: linear-gradient(135deg, #1aad19 0%, #07c160 100%); border-radius: 12px; line-height: 240px; color: #fff; font-size: 80px; text-align: center;">▶</span>
    </a>
  </p>'''}
  <p style="text-align: center; margin: 16px 0 0 0;">
    <a href="{video_url}" target="_blank" style="display: inline-block; padding: 14px 36px; background: #07c160; color: #fff; text-decoration: none; border-radius: 24px; font-size: 16px; font-weight: 500;">▶ 点击播放视频</a>
  </p>
  <p style="text-align: center; color: #999; font-size: 12px; margin: 8px 0 0 0;">(微信内点击会唤起内置播放器)</p>
</section>
...
示例:
  ...
  python3 publish_video_article.py --file ep01.mp4 --title "EP01" \\
    ...
    p.add_argument("--cover", default="", help="封面图本地路径(可选,留空则尝试用 ffmpeg 抽视频首帧)")
    p.add_argument("--no-frame", action="store_true",
                   help="不用 ffmpeg 抽帧(避免用 video_url 做默认封面)")
    ...
    try:
        ...
        # 3. 生成 HTML (含封面图,如有)
        cover = args.cover
        if not cover and not args.no_frame and not args.no_upload:
            # 尝试从本地视频抽帧作封面
            try:
                cover = extract_first_frame(args.file)
                print(f"  ③ 已抽取首帧作封面: {cover}")
            except Exception as e:
                print(f"  ⚠️  抽帧失败(继续): {e}")
                cover = ""

        html = build_video_html(video_url, args.title, args.introduction, args.extra, poster=cover)
        if args.print_html:
            print(html)
